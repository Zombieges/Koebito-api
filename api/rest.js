...
rest.prototype.handleRoutes = function(router) {
    ...
    router.get('/voices', function(req, res) {
        ...
        req.mysql.query("select v.kind,ownerId,v.soundUrl,v.title,u.userId,u.userName,u.imageUrl from voices as v Inner join users as u on v.ownerId = u.userId and v.kind = " + req.query.kind, function(err, rows) {
            if(err) {
                res.json({"Error" : true, "Message" : "Error executing MySQL query","Error Content" : err});
            } else {
